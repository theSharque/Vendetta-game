...
// debug_it( "from=".$user['id']." type=".$type." uid=".$rid." arg1=".$arg1." arg2=".$arg2 );
    ...
    switch( $type ) {
      ...
      case 2: // Approve answer
        $photo = db_fetch_row( "SELECT user_id, id FROM photos WHERE status = 1 AND file = '$arg2'" );

        if( $photo['user_id'] ) {
          db_query( "DELETE FROM message WHERE type = 1 AND sender_id = {$photo['user_id']}" );

          if( $arg1 == 1 && $photo['id'] ) {
            db_query( "DELETE FROM photos WHERE status = 2 AND user_id = {$photo['user_id']}" );
            db_query( "UPDATE photos SET status = 2 WHERE id = {$photo['id']}" );
            db_query( "UPDATE users SET photo_id = {$photo['id']} WHERE id = {$photo['user_id']}" );
          } else {
            db_query( "DELETE FROM photos WHERE id = {$photo['id']}" );
          }
      ...
      case 3: // Invite to game
        if( $arg1 == 0 ) {
          $test = db_fetch_val( "SELECT u.game_id
                                   FROM users u
                             INNER JOIN games g ON g.id = u.game_id
                                  WHERE u.die = 0 AND g.status = 2 AND u.id = $rid", 'game_id' );
          if( $test == 0 ) {
            $arg1 = $user['game_id'];
            $arg2 = "";
            $send = true;
      ...
      case 4: // Leave or close game
        $test = db_fetch_row( "SELECT u.game_id, u.die, g.admin_id, g.status
                                 FROM users u
                           INNER JOIN games g ON g.id = u.game_id
                                WHERE u.id = {$user['id']}" );
        ...
        if( $test['admin_id'] == $user['id'] && $test['status'] == 1 ) {
          ...
          db_query( "UPDATE users u SET game_id = 0, die = 0 WHERE game_id = {$test['game_id']}" );
          ...
          if( $test['status'] == 1 ) {
            $rid = $test['admin_id'];
            $arg2 = $user['login'];
            db_query( "UPDATE users u SET game_id = 0, die = 0 WHERE id = {$user['id']}" );
            $send = true;
          } else {
            if( $test['die'] == 0 && $test['status'] == 2 ) {
      ...
      case 5: // Invite answer
        ...
        if( $arg1 == 1 && $game['id'] ) {
          ...
          if( count( $from ) == 2 && count( $user['location'] ) == 2 &&
              distance( $from[0], $from[1], $user['location'][0], $user['location'][1] ) < 100000 ) {
            if( $game['status'] != 1 || ( $game['start_type'] == 0 && $game['start_count'] && $game['in_game'] >= $game['start_count'] ) ) {
              ...
              if( $game['game_type'] == 0 ) {
                ...
                if( $game['in_game'] + 1 == $game['start_count'] ) {
      ...
      case 6: // ping user
        ...
        db_query( "UPDATE games SET location = '$arg2' WHERE admin_id = {$user['id']}" );

        if( $user['game_status'] == 2 ) {
          $game = explode( ':', $user['game_loc'] );
          if( count( $game ) == 2 && count( $user['location'] ) == 2 && 
              distance( $game[0], $game[1], $user['location'][0], $user['location'][1] ) > 200000 ) {
            killPlayer( $user['id'], 3 );
          }
        }
        break;
      ...
      case 11: // bsid update
        db_query( "UPDATE users SET bsid = '$arg2' WHERE id = {$user['id']}" );
        break;
    }
